...
def find_best_model(trial):
    ...
    layer_1 = tf.keras.layers.Dense(100, activation=trial.suggest_categorical("activation", args.hpopt_activation))(input_layer_flattened)
    ...
    output = tf.keras.layers.core.Dense(y_train.shape[1], activation=trial.suggest_categorical('activation', args.hpopt_activation), name='output',
    ...
    model_checkpoint = tf.keras.callbacks.ModelCheckpoint('%sclf_wa_mapping_trial%s.h5' % (args.checkpoint_dir, trial.number),
                                                       monitor='val_loss',
                                                       verbose=0,
                                                       # save_best_only=True,
                                                       save_weights_only=False,
                                                       mode='auto')
    lr_reducer = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=10, verbose=0, mode='auto',
