# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 12:52:14 2019
...
print(outliers)
dfFree1 = dfOrigin.drop(dfOrigin.index[outliers])
...
scaler = MinMaxScaler()
dfFree1 = pd.DataFrame(scaler.fit_transform(dfFree1), columns=dfFree1.columns)
...
X_train=dfFree1.tail(train_size)
X_test=dfFree1.head(dfFree1.shape[0]-train_size)
...
outliers2 = df_error.index[df_error.reconstruction_error > 0.1].tolist()

dfFree2 = dfOrigin2.drop(dfOrigin2.index[outliers2])
...
scaler = MinMaxScaler()
dfFree2 = pd.DataFrame(scaler.fit_transform(dfFree2), columns=dfFree2.columns)
...
X_train=dfFree2.tail(train_size)
X_test=dfFree2.head(dfFree2.shape[0]-train_size)
...
outliers3 = df_error.index[df_error.reconstruction_error > 0.1].tolist()

dfFree3 = dfOrigin3.drop(dfOrigin3.index[outliers3])

# Da mit den letzten Tausend trainiert wurde, wurden nicht die Anomalien 
# aufgelistet und entfernt bei diesen Tausen.
# Deswegen werden die ersten Tausend noch entfernt.
dfRealFree=dfFree3.head(dfFree3.shape[0]-1000)


dfRealFree.to_csv('anomalienFrei.csv')
