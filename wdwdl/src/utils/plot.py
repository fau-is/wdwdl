...
def reconstruction_error(args, df, col, threshold):
    ...
    # threshold
    label = r"$t = %s$" % str(round(threshold, 5))
...
def export_reconstruction_error(args, df, col, threshold):
    """ Exports the reconstruction error of as a csv file. """

    data = pandas.DataFrame()
    data['reconstruction_error'] = df[col]
    data.index.name = 'process_instance'
    data['threshold'] = threshold
    data.to_csv('results/re_' + args.data_set[:-4] + '.csv')
...
def confusion_matrix(label_ground_truth, label_prediction, args):
    ...
    fig.tight_layout()
    fig.savefig(str(args.result_dir + 'cm_' + args.data_set[:-4] + "_" + str(args.remove_noise) + "_" + str(args.remove_noise_threshold) + '.pdf'))
